      ...
      {questions.map((item, index) => (
        <div key={index} className="mb-6">
          ...
          <p className="text-gray-700 mb-3">
            En una calificación de 1 a {item.scale}, donde {item.scale}{" "}
            significa completamente capacitado y 1 absolutamente no
          </p>
          <div className="grid grid-cols-5  w-full">
            {[...Array(item.scale)].map((_, idx) => (
              ...
              >
                <label
                  className={`w-full text-center py-2 ${colors[idx % colors.length]} text-white font-bold border-b border-gray-300`}
                >
                  {idx + 1}
                </label>
                <input
